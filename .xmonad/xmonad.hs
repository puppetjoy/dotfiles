...
import Data.Map
import Data.Monoid
import Graphics.X11.Xlib (Dimension, setWindowBorderWidth)
...
isTelegramMediaViewer :: Query Bool
isTelegramMediaViewer =
    className =? "TelegramDesktop" <&&>
    title =? "Media viewer" <&&>
    isInProperty "_NET_WM_WINDOW_TYPE" "_KDE_NET_WM_WINDOW_TYPE_OVERRIDE"
...
setBorderWidth' width window = withDisplay $ \display ->
    io $ setWindowBorderWidth display window width

suppressTelegramMediaViewerBorders :: X ()
suppressTelegramMediaViewerBorders =
    allWindowsByType isTelegramMediaViewer >>= mapM_ (setBorderWidth' 0)
...
manageHook' = composeOne
    ...
    , isTelegramMediaViewer                   -?> doFullFloat <+> doWindowAction (setBorderWidth' 0)
    , isSplash                                -?> doIgnore
    , isTooltip                               -?> doIgnore
