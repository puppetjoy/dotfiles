...
# Set path
typeset -U path
for dir in /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin /usr/glue/bin /usr/glue/sbin $HOME/bin $HOME/local/bin $HOME/.local/bin $HOME/streamux/bin; do
    [[ -e $dir ]] && path=($dir $path)
done
