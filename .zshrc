...
# 't' because if tmux is unavailable, then it will return a non-zero
# exit code and zsh will continue as a fallback; and when tmux does run
# and exits cleanly, I still want ~/.zlogout evaluated.
if [[ ! $TMUX && $CODEX_SHELL != 1 ]]; then
    t 2>/dev/null && exit
fi
